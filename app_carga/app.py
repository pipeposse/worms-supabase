...
def inject_global_css():
    ...
    <style>
      ...
      /* El rótulo del botón NO es texto suelto: Streamlit lo envuelve en un párrafo
         adentro del botón. La regla global de párrafos (color var(--ink-2)) le ganaba a
         la herencia del color del botón, así que el "Entrar" de los botones azules salía
         oscuro sobre azul y no se leía (dirección, 14/09/2026). Con inherit el rótulo
         toma siempre el color del botón: blanco en el primario, tinta en el secundario,
         gris en el terciario, y el del hover cuando corresponde.
         OJO: nunca escribir etiquetas HTML dentro de estos comentarios; el CSS se inyecta
         con st.html y el sanitizador descarta TODO el bloque si encuentra una. */
...
def puede_seccion(sec):
    ...
    if sec == "ADMIN" and USR.get("rol") != "ADMIN":
        return False
    if sec == "MEJORAS":
        return True  # Mejoras/soporte es para TODOS los usuarios, sin importar sus accesos
...
if "MEJORAS" not in _ALLOWED_SECS:
    _ALLOWED_SECS.append("MEJORAS")  # Mejoras/soporte es para todos
_LOCKED_ONE = len([s for s in _ALLOWED_SECS if s != "MEJORAS"]) == 1
if _LOCKED_ONE and st.session_state.section != "MEJORAS":
    st.session_state.section = [s for s in _ALLOWED_SECS if s != "MEJORAS"][0]
    ...
    if not _LOCKED_ONE:
        ...
        if st.session_state.section == "MEJORAS":
            if st.button("← Volver a mi sección", use_container_width=True, key="sb_mj_back"):
                st.session_state.section = [s for s in _ALLOWED_SECS if s != "MEJORAS"][0]
                st.rerun()
        elif st.button("🛠️ Mejoras y problemas", use_container_width=True, key="sb_mejoras"):
            st.session_state.section = "MEJORAS"
            st.rerun()
    if USR.get("sector"):
...
if st.session_state.section != "CARGAS":
    ...
    if st.session_state.section == "LAB":
        ...
        try:
            from remitos_section import render as _render_remitos
            _render_remitos(USR, conectar)
        except Exception as _e:
            import traceback as _tbr
            st.error(f"No se pudo cargar Remitos: {_e}")
            with st.expander("Detalle"):
                st.code(_tbr.format_exc())
    ...
    elif st.session_state.section == "ANALISIS":
        try:
            import analisis_reacciones as _anr
            _anr.render(USR, cat, conectar)
        except Exception as _e:
            import traceback as _tban
            st.error(f"No se pudo cargar Análisis de reacciones: {_e}")
            with st.expander("Detalle"):
                st.code(_tban.format_exc())
        ...
        if _ip_view.startswith("📈"):
            ...
            try:
                ...
                disponibilidad_descarga.render(USR, cat, conectar, "PLANTA")
            except Exception as _e:
                st.exception(_e)
    ...
    elif st.session_state.section == "MEJORAS":
        try:
            from mejoras_section import render as _render_mejoras
            _render_mejoras(USR, cat, conectar, lab_conn=_lab_conn)
        except Exception as _e:
            import traceback as _tbtk
            st.error(f"No se pudo cargar Mejoras y problemas: {_e}")
            with st.expander("Detalle"):
                ...
                if _usa_multiselect_mp:
                    ...
                    if _opts_mp_tl[0] == "AFE-S":
                        st.info("♻️ **Reproceso de AFE-S calidad D**: al desgomado sólo debería "
                                ...
                                "> 150 ppm). El A/B/C es producto vendible: se vende, no se "
                                "reprocesa.")
                else:
