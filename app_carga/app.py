...
if st.session_state.section is None:
    ...
    tiles = [
        ...
        ("📦", "Stock y movimientos", "Libro mayor de movimientos, stock estimado en tiempo real y reconciliación. Todo descargable.", "STOCK", "land_stock", False),
    ]
...
if st.session_state.section != "CARGAS":
    ...
    if st.session_state.section == "LAB":
        ...
        _lab_view = st.radio("Vista", ["📊 Resultados", "➕ Carga / Edición"],
...
"""
        ...
        if _panel.empty:
            ...
            def _valid(r):
                # Validación: % llenado = litros / capacidad de cubicaje (o nominal). Alerta si supera.
                cap = r.get("cap_aforo")
                if pd.isna(cap) or not cap:
                    cap = r.get("capacidad_litros")
                if pd.isna(r["_litros"]):
                    return "—"
                if pd.isna(cap) or not cap:
                    return "sin cubicaje"
                pct = r["_litros"] / float(cap) * 100.0
                if pct > 102:
                    return f"⚠️ supera ({pct:.0f}%)"
                if pct < -1:
                    return "⚠️ negativo"
                return f"✔️ {pct:.0f}%"
            d["Validación"] = d.apply(_valid, axis=1)
            ...
            _n_sup = int(d["Validación"].astype(str).str.startswith("⚠️").sum())
            _n_nom = int((d["Estado"] == "⛔ No medible").sum())
            if (_n_sin + _n_vieja + _n_sup + _n_nom) > 0:
                _bits = []
                if _n_sup:   _bits.append(f"**{_n_sup}** superan capacidad / valor inválido")
            ...
            _t = _t[["codigo", "nombre", "tipo_tanque", "sector", "producto_principal", "Medidor", "wedo_label",
                     "intervalo_real_min", "nivel_pct_actual", "_litros", "_tn", "capacidad_litros", "ultima_medicion", "Estado", "Validación"]].rename(columns={
            ...
            st.caption("**Estado** = frescura de la medición (sensor al día / manual hoy-días-vieja / sin medición / no medible). "
                       "**Validación** = `% llenado = litros ÷ capacidad de cubicaje`; ⚠️ si supera ~100% (medición inconsistente). "
                       "La capacidad sale del aforo (`dim_tanque_aforo`); si el tanque no tiene cubicaje, dice *sin cubicaje*.")
            ...
            st.divider()
            g_cargar, g_aforo, g_editar, g_hist = st.tabs(["✍️ Cargar medición (manual)", "📏 Medir por cm (aforo)", "✏️ Editar tanque", "📈 Histórico / variación"])
            ...
            with g_cargar:
                ...
                if _man.empty:
                    ...
                    if _mm.empty:
                        ...
                        _pcod = cc1.selectbox("Producto medido", _plist, index=_plist.index(_defp), key="tq_prod_c")
                        _lts = cc2.number_input("Litros medidos", 0.0, 5_000_000.0, step=100.0, value=0.0, key="tq_lts_c")
                        _densp = float(_prods[_prods["codigo_producto"] == _pcod]["dens"].iloc[0])
                        _kg = _lts * _densp
                        _ocup = (_lts / _cap * 100) if _cap else 0.0
                        m1, m2, m3 = st.columns(3)
                        m1.metric("Equivale a", f"{_kg/1000:,.2f} TN")
                        m2.metric("En kg", f"{_kg:,.0f} kg")
                        m3.metric("Ocupación", f"{_ocup:.0f}%" if _cap else "—")
                        st.caption(f"Densidad {_densp:g} kg/L · capacidad {int(_cap):,} L")
                        cf, ch = st.columns(2)
                        ...
                        if st.button("💾 Guardar medición", type="primary", use_container_width=True, key="tq_save_c"):
                            if (_lts or 0) <= 0:
                                st.error("Cargá los litros (mayor a 0).")
                            else:
                                try:
                                    with conectar(USR["id_usuario"]) as (conn, audit):
                                        with conn.cursor() as cur:
                                            ...
                                            cur.execute("INSERT INTO produccion.fact_stock_tanque "
                                                        ...
                                                        (_idt, _pid, _medido.isoformat(), float(_lts), float(_kg),
                                                         int(USR["id_usuario"]), _obs or None))
                                        audit.log("I", "fact_stock_tanque", _idt, {"producto": _pcod, "litros": float(_lts)})
                                    st.success(f"Medición guardada: {_row['nombre']} · {_pcod} · {_lts:,.0f} L ({_kg/1000:,.2f} TN).")
                                    cat.clear()
                                except Exception as e:
                                    st.exception(e)
        ...
        try:
            ...
            _render_iniciar(USR, cat, conectar)
        except Exception as _e:
