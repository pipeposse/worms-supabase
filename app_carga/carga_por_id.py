...
def render(USR, cat, conectar, etapas_de_proceso=None, params_proceso=None):
    ...
    det = cat(
        ...
        "       b.tiempo_estimado_horas, b.parametros_proceso, b.corriente, "
    ...
    _corr = (str(d.get("corriente") or "")).upper()
    if _corr == "VEGETAL":
        st.markdown("<div style=\'background:#dcfce7;border:2px solid #16a34a;border-radius:12px;"
                    "padding:14px;text-align:center;font-size:1.5rem;font-weight:800;color:#166534;margin:6px 0\'>"
                    "🌱 CORRIENTE VEGETAL — aceites (AG)</div>", unsafe_allow_html=True)
    elif _corr == "ANIMAL":
        st.markdown("<div style=\'background:#ffedd5;border:2px solid #c2410c;border-radius:12px;"
                    "padding:14px;text-align:center;font-size:1.5rem;font-weight:800;color:#7c2d12;margin:6px 0\'>"
                    "🐄 CORRIENTE ANIMAL — sebos</div>", unsafe_allow_html=True)
    ...
    _gli_l = float(params.get("glicerina_fresca_l") or 0) + float(params.get("glicerina_recup_l") or 0)
    litros_carga = litros_mp + _gli_l
    pct_llen = (litros_carga / cap_l * 100.0) if cap_l else 0.0

    c5, c6, c7, c8 = st.columns(4)
    c5.metric("MP a cargar", f"{litros_mp:,.0f} L", f"+ glicerina {_gli_l:,.0f} L")
    c6.metric("Capacidad reactor", f"{cap_l:,.0f} L")
    c7.metric("Llenado del reactor", f"{pct_llen:.0f}%", help="MP + glicerina fresca + recuperada")
