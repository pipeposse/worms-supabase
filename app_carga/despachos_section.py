...
def _puntaje_calidad(r, spec):
    """Suma de parámetros relativos a la spec (menor = mejor). Sin lab = 990."""
    rr = []
    for c, k in (("acidez", "acidez"), ("agua_sedimento", "ays"),
                 ("azufre", "azufre"), ("fosforo", "fosforo")):
        lim = spec.get(k)
        v = r.get(c) if hasattr(r, "get") else None
        if lim and v is not None and pd.notna(v):
            rr.append(float(v) / float(lim))
...
def _lab_corto(r):
    def _f(v, dec):
        return ("%.*f" % (dec, float(v))) if v is not None and pd.notna(v) else "?"
    return "ac %s · P %s · S %s · AyS %s" % (
        _f(r.get("acidez"), 2), _f(r.get("fosforo"), 0),
        _f(r.get("azufre"), 0), _f(r.get("agua_sedimento"), 2))


def _orden_opciones(tp, fam, spec):
    """Claves de TODA la familia, ordenadas por calidad: base primero, después AFE-S,
    después el resto; dentro de cada grupo, mejor calidad primero y sin lab al final."""
    df = tp.copy()
    ...
    df["_rol"] = up.map(_rol)
    df["_q"] = df.apply(lambda r: _puntaje_calidad(r, spec), axis=1)
    df = df.sort_values(["_rol", "_q", "nombre"], ascending=[True, True, True])
    out, seen = [], set()
    for v in df["clave"]:
        if v not in seen:
            seen.add(v)
...
def _info_lineas(df, tks, spec):
    """Columnas informativas (solo lectura) para el editor: Disp. (L) y Calidad."""
    m = tks.drop_duplicates(subset=["clave"]).set_index("clave")
    disp, cal = [], []
    for v in df["Tanque"]:
        k = _clave_de(v, tks)
        if k and k in m.index:
            r = m.loc[k]
            d = r["litros_actual"]
            disp.append(float(d) if pd.notna(d) else 0.0)
            cal.append("%s %s" % (_sem_calidad(r, spec), _lab_corto(r)))
        else:
            disp.append(_NAN)
            cal.append("")
    out = df.copy()
    ...
    try:
        ...
        for _, r in df.iterrows():
            ...
            try:
                ...
                _l = 0.0
            _p.append("%s=%.0f" % (_t, _l))
        return "|".join(sorted(_p))
    except Exception:
        return ""


def _lineas_actuales(ss, tks):
    """La carga vigente: lo último editado en la grilla si existe, si no dsp_lineas."""
    cur = ss.get("_dsp_last_ed")
    if cur is None or not isinstance(cur, pd.DataFrame):
        cur = ss.get("dsp_lineas")
    if cur is None or not isinstance(cur, pd.DataFrame):
        cur = _base_vacia(_COLS_ED)
...
def _selector_tanques(ss, tp, fam, spec, tks, conectar, USR, inc_vacios,
    ...
    _modos = ["📋 Lista", "🧱 Tarjetas"]
    try:
        _modo = st.segmented_control("Modo de selección", _modos, default=_modos[0],
                                     key="dsp_selmodo_sc", label_visibility="collapsed")
    except Exception:
        _modo = st.radio("Modo de selección", _modos, horizontal=True, key="dsp_selmodo_rd")
    _modo = _modo or _modos[0]
    if _modo.startswith("🧱"):
        _selector_tarjetas(ss, tp, fam, spec, tks, conectar, USR, inc_vacios)
        return
    _selector_lista(ss, tp, fam, spec, tks, conectar, USR, inc_vacios, prod_cod, lit_obj)
...
def _selector_lista(ss, tp, fam, spec, tks, conectar, USR, inc_vacios, prod_cod, lit_obj):
    ...
    for _, r in cur.iterrows():
        ...
        if _t:
            try:
                _map_l[_t] = float(r.get("Litros") or 0)
            except Exception:
                _map_l[_t] = 0.0
    ...
    for _, r in _pool.iterrows():
        _cl = str(r["clave"])
        _disp = float(r["litros_actual"]) if pd.notna(r["litros_actual"]) else 0.0
        _lit = float(_map_l.get(_cl, 0.0))
        _filas.append({
            "Usar": _cl in _map_l,
            "Litros": round(_lit, 0),
            "Tanque": str(r["nombre"]),
            ...
            "Acidez %": (float(r["acidez"]) if pd.notna(r["acidez"]) else None),
    ...
    # Si la carga cambió por FUERA de la lista (sugerencia, propuesta, deshacer,
    # borrador…), se rehace el editor para que refleje esos litros; si el cambio lo
    # hizo la propia lista, se conserva el estado (no se pierde el foco al tipear).
    _fir = _firma_lineas(cur)
    if ss.get("_dsp_lst_fir") != _fir:
        ss["_dsp_lst_fir"] = _fir
        ss["dsp_lst_nonce"] = int(ss.get("dsp_lst_nonce") or 0) + 1
        ...
        column_config={
            "Usar": st.column_config.CheckboxColumn("Usar", width="small",
                                                    help="Tildá para sumarlo a la carga con "
                                                         "todo lo disponible."),
            "Litros": st.column_config.NumberColumn("Litros a cargar", min_value=0.0,
    ...
    # ----- de la tabla a la carga -----
    _edl = _edl.copy()
    _edl["_clave"] = _df["_clave"].values
    _nuevas = []
    for _, r in _edl.iterrows():
        if not bool(r["Usar"]):
            continue
        _cl = str(r["_clave"])
        try:
            ...
            _l = 0.0
        # tildado recién y sin litros → entra a TOPE (es lo que pidieron los operarios)
        if _l <= 0 and _cl not in _map_l:
            _l = float(int(float(r["Disp. (L)"] or 0) // 10) * 10)
        _prev = cur[cur["Tanque"].astype(str) == _cl]
        _fila = {"Tanque": _cl, "Litros": round(_l, 0), "Acidez %": _NAN,
                 "Fósforo ppm": _NAN, "Azufre ppm": _NAN, "AyS %": _NAN}
        if not _prev.empty:      # se conservan los overrides de lab tipeados a mano
            for _c in ("Acidez %", "Fósforo ppm", "Azufre ppm", "AyS %"):
                _fila[_c] = _prev.iloc[0].get(_c, _NAN)
        _nuevas.append(_fila)
    _df_new = (pd.DataFrame(_nuevas).reindex(columns=_COLS_ED) if _nuevas
               else _base_vacia(_COLS_ED))
    if _firma_lineas(_df_new) != _fir:
        ss["dsp_lineas"] = _df_new
        ss["_dsp_last_ed"] = _df_new.copy()
        ss["_dsp_lst_fir"] = _firma_lineas(_df_new)
        ss["dsp_ed_nonce"] = int(ss.get("dsp_ed_nonce") or 0) + 1   # refresca la grilla
        try:
            _borr_guardar(conectar, USR, _df_new)
        except Exception:
            pass
    ...
    # ----- totales y ponderados de lo seleccionado -----
    _res = _resolver(_df_new, tks, prod_cod)
    b1, b2, b3 = st.columns([1.1, 1.1, 1.4])
    if _res is None or _res.empty:
        b1.metric("Seleccionado", "0 L")
        st.caption("Todavía no hay tanques tildados.")
    else:
        _tl = float(_res["Litros"].sum())
        _tk_ = float(_res["kg"].sum()) / 1000.0
        b1.metric("Seleccionado", "%s L" % "{:,.0f}".format(_tl),
                  "%d tanque(s)" % len(_res))
        b2.metric("Total", "%.2f TN" % _tk_)
        if lit_obj and float(lit_obj) > 0:
            _pc = 100.0 * _tl / float(lit_obj)
            b3.metric("% del objetivo", "%.1f %%" % _pc,
                      "%s L" % "{:,.0f}".format(_tl - float(lit_obj)),
                      help="Objetivo = contenedores × litros por contenedor (%s L)."
                           % "{:,.0f}".format(float(lit_obj)))
        _pp = []
    ...
    if c1.button("🔋 Tildados al tope", key="dsp_lst_tope", use_container_width=True,
                 help="Pone en cada tanque tildado todos sus litros disponibles."):
        _mm = tks.drop_duplicates(subset=["clave"]).set_index("clave")["litros_actual"]
        _nl = []
        for _, _r in cur.iterrows():
            _k2 = _clave_de(_r.get("Tanque"), tks)
            _r = _r.copy()
            if _k2 and _k2 in _mm.index and pd.notna(_mm[_k2]):
...
def _selector_tarjetas(ss, tp, fam, spec, tks, conectar, USR, inc_vacios):
    """Panel visual de tanques (segunda opción): tarjetas por producto, click = entra/sale.
    ...
    _lleno = c2.checkbox("🔋 Llenado total al agregar", value=True, key="dsp_card_full",
                         help="Cada click carga TODO lo disponible del tanque. "
                              "Desmarcá para agregarlo en 0 y poner los litros a mano.")
    cur = _lineas_actuales(ss, tks)
    if c3.button("⛽ Llenar líneas al máximo", key="dsp_fill_all", use_container_width=True,
                 help="Pone en cada línea ya cargada todos los litros disponibles del tanque."):
        _mm = tks.drop_duplicates(subset=["clave"]).set_index("clave")["litros_actual"]
        _nl = []
        for _, _r in cur.iterrows():
            _k = _clave_de(_r.get("Tanque"), tks)
            if _k and _k in _mm.index and pd.notna(_mm[_k]):
                _r = _r.copy()
                _r["Litros"] = float(int(float(_mm[_k]) // 10) * 10)
            _nl.append(_r)
        if _nl:
    ...
    if _pool.empty:
        st.info("No hay tanques de %s con stock disponible." % _fsel)
        return
    _pool["_q"] = _pool.apply(lambda r: _puntaje_calidad(r, spec), axis=1)
    _pool["_x"] = (~_pool["sector"].astype(str).str.strip()
                   .isin(list(SECTORES_PRIORIDAD))).astype(int)
    _pool = _pool.sort_values(["_x", "_q", "litros_actual"], ascending=[True, True, False])
    st.caption("2 · Ordenados de **mejor a peor calidad**; los de sectores no prioritarios "
               "van al final. Banda: "
    ...
    for _rw in _fil:
        ...
        for _j, (_, r) in enumerate(_rw.iterrows()):
            with _cc[_j].container(border=True):
                _en = str(r["clave"]) in _sel
                _disp = float(r["litros_actual"]) if pd.notna(r["litros_actual"]) else 0.0
                _cap = float(r["capacidad_litros"]) if pd.notna(r["capacidad_litros"]) else 0.0
                _pct = int(min(100, max(0, 100.0 * _disp / _cap))) if _cap else 0
                st.markdown("**%s**%s" % (r["nombre"], " · ✅ en la carga" if _en else ""))
                st.progress(_pct, text="%s L disp." % "{:,.0f}".format(_disp))
                st.caption("%s %s" % (_sem_calidad(r, spec), _lab_corto(r)))
                if _en:
                    _lbl = "➖ Quitar"
                elif _lleno and _disp > 0:
                    _lbl = "➕ Todo (%s L)" % "{:,.0f}".format(float(int(_disp // 10) * 10))
                else:
                    _lbl = "➕ Agregar"
                if st.button(_lbl, key="dsp_card_%d" % int(r["id_tanque"]),
                             use_container_width=True):
                    df = cur.copy()
                    if _en:
                        df = df[df["Tanque"].astype(str) != str(r["clave"])]
                    else:
                        _l = float(int(_disp // 10) * 10) if _lleno else 0.0
                        df = pd.concat([df, pd.DataFrame([{
                            "Tanque": r["clave"], "Litros": _l, "Acidez %": _NAN,
                            "Fósforo ppm": _NAN, "Azufre ppm": _NAN, "AyS %": _NAN}])],
                            ignore_index=True)
                    df = df[~(df["Tanque"].isna()
                              | (df["Tanque"].astype(str).str.strip() == ""))]
                    df = df.reindex(columns=_COLS_ED)
                    _lineas_set(ss, df)
...
def _sugerir(tks, prod_cod, litros_obj, spec, prods=None, l_base=None, maximizar=False,
    ...
    * la cantidad de AFE-S C+D que se logra colocar (el AFE-S A+B escasea y hay que
    ...
    if formulado and lb_min and float(lb_min) > 0:
        ...
        if lb + 0.5 < _piso:
            _lb_antes = lb
            _lb_piso = float(int(_piso // 10) * 10)
            _ok_piso = _cumple(_armar_mezcla(_lb_piso)[0], tol)
            if _ok_piso or _lb_antes <= 0 or not _cumple(_armar_mezcla(_lb_antes)[0], tol):
                lb = _lb_piso
                ...
                # El mínimo por contenedor es una REGLA con excepción, no un número fijo:
                # si con el piso la spec NO cierra pero con menos base sí, gana la spec
                # y queda avisado para validar el desvío con dirección.
                notas.append("⚠️ %s por DEBAJO del mínimo: quedan %s L y la regla pide %s L. "
                             "Con el piso completo la spec NO cierra con estos diluyentes — "
                             "validá el desvío con dirección o cambiá los tanques de AFE."
                             % (base_cod, "{:,.0f}".format(_lb_antes), "{:,.0f}".format(_piso)))
...
def _propuestas(tks, prod_cod, litros_obj, spec, prods, tol, lb_min=0.0):
    ...
    if not a_df.empty:
        ...
        if sa["lb"] > _piso + 20:
            # B intermedio: punto medio entre el mínimo por regla y el máximo de A.
            lb_b = float(int((_piso + (sa["lb"] - _piso) / 2.0) // 10) * 10)
            ...
            # A quedó pegada al piso: la spec no banca más base. B = spec primero,
            # dejando caer el mínimo por contenedor (la propuesta lo avisa).
            b_df, _m = _sugerir(tks, prod_cod, litros_obj, spec, prods, None,
                                maximizar=True, tol=tol, lb_min=0.0)
            if not b_df.empty:
...
def _monitor_baja(USR, cat, conectar):
    ...
    d = cat("SELECT d.id_despacho, d.titulo, d.fecha_despacho, d.estado, d.n_contenedores, "
            ...
            " (SELECT COUNT(*) FROM produccion.fact_despacho_linea l "
            "   WHERE l.id_despacho=d.id_despacho) AS n_lin, "
            " (SELECT COUNT(*) FROM produccion.fact_despacho_linea l "
            "   WHERE l.id_despacho=d.id_despacho AND l.verif_estado IS NOT NULL) AS n_verif, "
            " (SELECT COUNT(*) FROM produccion.fact_despacho_ticket t "
            "   WHERE t.id_despacho=d.id_despacho) AS n_tk, "
            " (SELECT COALESCE(SUM(t.kg),0) FROM produccion.fact_despacho_ticket t "
            "   WHERE t.id_despacho=d.id_despacho) AS kg_tk, "
            " (SELECT COUNT(*) FROM produccion.fact_movimiento_stock m "
            "   WHERE m.id_despacho=d.id_despacho AND NOT m.anulado) AS n_mov, "
            " (SELECT COUNT(*) FROM produccion.fact_movimiento_stock m "
            "   WHERE m.id_despacho=d.id_despacho AND NOT m.anulado "
            "     AND m.estado_mov='EJECUTADO') AS n_ejec, "
            " (SELECT COALESCE(SUM(m.kg),0) FROM produccion.fact_movimiento_stock m "
            "   WHERE m.id_despacho=d.id_despacho AND NOT m.anulado) AS kg_mov "
            "FROM produccion.fact_despacho d "
            "WHERE d.estado IN ('CONFIRMADO','DESPACHADO') "
    ...
    # --- evidencia física: caída medida por tanque, despacho por despacho ---
    med = cat("SELECT l.id_despacho, l.id_tanque, COALESCE(t.nombre,'—') AS tanque, "
              "       l.producto_codigo, SUM(l.litros) AS litros_plan, "
              "  (SELECT s.litros FROM produccion.fact_stock_tanque s "
              "    WHERE s.id_tanque=l.id_tanque "
              "      AND s.medido_en < d.fecha_despacho::timestamptz "
              "    ORDER BY s.medido_en DESC LIMIT 1) AS l_antes, "
              "  (SELECT s.litros FROM produccion.fact_stock_tanque s "
              "    WHERE s.id_tanque=l.id_tanque "
              "      AND s.medido_en >= (d.fecha_despacho::timestamptz + interval '1 day') "
              "    ORDER BY s.medido_en ASC LIMIT 1) AS l_despues "
              "FROM produccion.fact_despacho d "
              "JOIN produccion.fact_despacho_linea l ON l.id_despacho=d.id_despacho "
              "LEFT JOIN produccion.dim_tanque t ON t.id_tanque=l.id_tanque "
              "WHERE d.estado IN ('CONFIRMADO','DESPACHADO') "
              "  AND d.fecha_despacho >= current_date - %s "
              "GROUP BY l.id_despacho, l.id_tanque, t.nombre, l.producto_codigo, "
              "         d.fecha_despacho", (int(_dias),))
    if med is None:
        med = pd.DataFrame(columns=["id_despacho", "id_tanque", "tanque", "producto_codigo",
                                    "litros_plan", "l_antes", "l_despues"])
    med = med.copy()
    for c in ("litros_plan", "l_antes", "l_despues"):
        med[c] = pd.to_numeric(med[c], errors="coerce")
    # caída medida: sólo cuenta si hay las dos mediciones y el tanque NO se recargó
    med["caida"] = med["l_antes"] - med["l_despues"]
    med["_estado_med"] = med.apply(
        lambda r: ("sin medición posterior" if pd.isna(r["l_despues"])
                   else ("sin medición previa" if pd.isna(r["l_antes"])
                         else ("recargado (no concluyente)" if r["caida"] < 0
                               else "medido"))), axis=1)

    def _res_med(idd):
        m = med[med["id_despacho"] == idd]
        if m.empty:
            return 0, 0, 0.0, 0.0
        _ok = m[m["_estado_med"] == "medido"]
        return (int(len(_ok)), int(len(m)), float(_ok["caida"].sum()),
                float(_ok["litros_plan"].sum()))

    _filas = []
    for _, r in d.iterrows():
        _idd = int(r["id_despacho"])
        _nok, _ntot, _caida, _plan_ok = _res_med(_idd)
        _tk_ok = r["n_tk"] >= max(1.0, r["n_contenedores"])
        _ej = r["n_ejec"] > 0
        # semáforo del ciclo completo
        if _tk_ok and _ej:
            _sem = "🟢 Baja cerrada"
        elif _tk_ok and not _ej:
            _sem = "🟡 Salió, falta cerrar"
        elif r["n_tk"] > 0:
            _sem = "🟡 Saliendo (parcial)"
        elif _caida > 0:
            _sem = "🟠 Bajó el tanque, sin tickets"
        else:
        ...
        _filas.append({
            ...
            "Ledger": ("%d mov · %s" % (int(r["n_mov"]),
                                        "EJECUTADO" if _ej else "planificado")),
            "Ledger (t)": float(r["kg_mov"]) / 1000.0,
            "Portería": "%d/%d tickets" % (int(r["n_tk"]), int(r["n_contenedores"])),
            "Pesado (t)": float(r["kg_tk"]) / 1000.0,
            "Medición": ("%d/%d tanques" % (_nok, _ntot)) if _ntot else "—",
            "Caída medida (L)": _caida if _nok else None,
            "Plan de esos (L)": _plan_ok if _nok else None,
            "Cobertura": (round(100.0 * _caida / _plan_ok, 0)
                          if (_nok and _plan_ok > 0) else None),
            "Verificó planta": "%d/%d" % (int(r["n_verif"]), int(r["n_lin"])),
            "_id": _idd,
        })
    _df = pd.DataFrame(_filas)
...
def _analisis(USR, cat):
    ...
    _pp = (d.groupby("producto_codigo")
             .agg(**{"Órdenes de venta": ("id_despacho", "count"), "TN": ("tn_total", "sum"),
...
def _armar(USR, cat, conectar):
    ...
    with st.expander("🧪 Actualizar parámetros de lab de un tanque acá mismo"):
        st.caption("Pisa el último análisis del tanque — lo mismo que carga Laboratorio — y "
                   "queda en el historial como AJUSTE_MANUAL con tu usuario. Usalo cuando el "
                   "lab pasó valores nuevos y todavía no están en el sistema.")
        _opq = _tp.apply(lambda r: "%s · %s" % (r["nombre"],
                                                str(r.get("producto_principal") or "—")),
                         axis=1).tolist()
        _selq = st.selectbox("Tanque", _opq, key="dsp_lab_tk")
        _rq = _tp.iloc[_opq.index(_selq)]
        st.caption("Actual: %s %s" % (_sem_calidad(_rq, spec), _lab_corto(_rq)))
        _q1, _q2, _q3, _q4 = st.columns(4)
        _qa = _q1.number_input("Acidez %", min_value=0.0, step=0.1, format="%.2f",
                               value=float(_rq["acidez"]) if pd.notna(_rq["acidez"]) else 0.0,
                               key="dsp_lab_ac")
        _qp = _q2.number_input("Fósforo ppm", min_value=0.0, step=5.0, format="%.1f",
                               value=float(_rq["fosforo"]) if pd.notna(_rq["fosforo"]) else 0.0,
                               key="dsp_lab_p")
        _qs = _q3.number_input("Azufre ppm", min_value=0.0, step=5.0, format="%.1f",
                               value=float(_rq["azufre"]) if pd.notna(_rq["azufre"]) else 0.0,
                               key="dsp_lab_s")
        _qy = _q4.number_input("AyS %", min_value=0.0, step=0.1, format="%.2f",
        ...
        if st.button("💾 Guardar parámetros del tanque", key="dsp_lab_go",
            ...
            try:
                with conectar(USR["id_usuario"]) as (conn, audit):
                    with conn.cursor() as cur:
                        cur.execute("SELECT id_producto_principal FROM produccion.dim_tanque "
                                    "WHERE id_tanque=%s", (int(_rq["id_tanque"]),))
                        _rp = cur.fetchone()
                        if not _rp or _rp[0] is None:
                            raise RuntimeError("el tanque no tiene producto principal asignado")
                        cur.execute("SELECT set_config('app.param_origen','AJUSTE_MANUAL',true)")
                        _xtra = json.dumps({"param_manual_desde": "despachos",
                                            "usuario": USR.get("nombre") or USR["id_usuario"]},
                                           default=str)
                        cur.execute(
                            "INSERT INTO produccion.fact_param_tanque "
                            "(id_tanque, id_producto, evaluado, ultima_evaluacion_ts, "
                            " acidez_pct, agua_pct, ppm_azufre, ppm_fosforo, "
                            " parametros_extra, actualizado_en) "
                            "VALUES (%s,%s,true,now(),%s,%s,%s,%s,%s::jsonb,now()) "
                            "ON CONFLICT (id_tanque, id_producto) DO UPDATE SET "
                            " evaluado=true, ultima_evaluacion_ts=now(), "
                            " acidez_pct=EXCLUDED.acidez_pct, agua_pct=EXCLUDED.agua_pct, "
                            " ppm_azufre=EXCLUDED.ppm_azufre, ppm_fosforo=EXCLUDED.ppm_fosforo, "
                            " parametros_extra = COALESCE(produccion.fact_param_tanque."
                            "parametros_extra,'{}'::jsonb) || EXCLUDED.parametros_extra, "
                            " actualizado_en=now()",
                            (int(_rq["id_tanque"]), int(_rp[0]), float(_qa), float(_qy),
                             float(_qs), float(_qp), _xtra))
                    audit.log("U", "fact_param_tanque", int(_rq["id_tanque"]),
                              {"acidez": float(_qa), "fosforo": float(_qp),
