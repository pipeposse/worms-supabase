...
def _control_rendimiento(USR, cat, conectar, idb):
    ...
    if _fin is None or pd.isna(_fin):
        ...
        _fin = _fq.iloc[0]["ts"] if (_fq is not None and not _fq.empty) else None
...
def _reacciones_terminadas(USR, cat, conectar):
    ...
    st.divider()
    _control_rendimiento(USR, cat, conectar, idb)
    st.divider()
...
def render(USR, cat, conectar, siguiente_identificador, H=None):
    ...
    if _grupo.startswith("⚙️"):
        ...
        if _admin.startswith("🛠️"):
            ...
            try:
                import editor_iniciadas
                editor_iniciadas.render(USR, cat, conectar)
            except Exception as _e:
                import traceback as _tb
                st.error("No se pudo cargar Edición de iniciadas: %s" % _e)
                with st.expander("🔧 Detalle técnico"):
                    st.code(_tb.format_exc())
    ...
    if _grupo.startswith("🎯"):
        try:
            ...
            _render_asig_afe(USR, cat, conectar, "PLANIFICACION")
        except Exception as _e:
            import traceback as _tb
    ...
    if _grupo.startswith("🔁"):
        try:
            ...
            _render_cc(USR, cat, conectar, "PLANIFICACION")
        except Exception as _e:
            import traceback as _tb
            st.error("No se pudo cargar Cambio de categoría: %s" % _e)
            with st.expander("🔧 Detalle técnico"):
                st.code(_tb.format_exc())
        return
    ...
    if _grupo.startswith("🏊"):
        try:
            import recuperado_piletas
            recuperado_piletas.render(USR, cat, conectar)
        except Exception as _e:
            import traceback as _tb
            st.error("No se pudo cargar Recuperado en piletas: %s" % _e)
            with st.expander("🔧 Detalle técnico"):
                st.code(_tb.format_exc())
        return
